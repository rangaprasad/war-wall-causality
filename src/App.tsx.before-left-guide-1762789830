...
function DagCanvas({
  ...
  }, [P, backdoorOK, frontdoorOK, labels]);
...
export default function App(){
  ...
  // setters for params
  const setBaseS = (v: number) => setP(p => ({ ...p, baseS: v }));
  const setAW    = (v: number) => setP(p => ({ ...p, aW: v }));
  const setAM    = (v: number) => setP(p => ({ ...p, aM: v }));
  const setNS    = (v: number) => setP(p => ({ ...p, nS: v }));
  const setBaseT = (v: number) => setP(p => ({ ...p, baseT: v }));
  const setBS    = (v: number) => setP(p => ({ ...p, bS: v }));
  const setNT    = (v: number) => setP(p => ({ ...p, nT: v }));
  const setCW    = (v: number) => setP(p => ({ ...p, cW: v }));
  const setThrT  = (v: number) => setP(p => ({ ...p, thrT: v }));
